...
struct Options {
	int RageNames = 0;
	string FriendList = "None";
	vector<string> ArrayFriends;

	int Drop_Vehicles = 0;
	bool handle_unload = false;
	bool Panic_DisableAll = false;

	bool ResetHP = false;
	bool ResetArmour = false;

	bool TpInNearVeh = false;
	bool TpInNearVeh_Passanger = false;
	bool TpToNearVeh = false;

	int ClassicGangWarMode = 0;

	int OpenMenuKey = 0x21;

	int Text_Shadow = 0;

	int Afk_Bot = 0;
	int PlayerListOnly = 0;
	int Save_Mode = 0;
	bool Engine_Draw = 0;
	int HotKeyFly = 0x999;
	int HotKeyTeleportToWayP = 0x999;
	int HotKeyNoClip = 0x999;
	int HotKeyResetHealth = 0x999;
	int HotKeyFlipVeh = 0x999;
	int HotKeyResetArmour = 0x999;
	int HotKeyAim = 0x999;
	int HotKeyAim2 = 0x999;
	int HotKeyVehImpulseIncreace = 0x999;
	int HotKeyVehImpulseDecrease = 0x999;
	int HotKeyVehImpulseConstant = 0x999;

	int WeaponSpawn1 = 0x1B06D571;
	int WeaponSpawn2 = 0x13532244;
	int WeaponSpawn3 = 0x83BF0278;
	int WeaponSpawn4 = 0x05FC3C11;

	int RemoveNearDeadCars = 0;
	int PickUpEsp = 0;
	int PlayerInfoCart = 0;
	int NoPlayerCollision = 0;
	int VehicleEsp = 0;
	int VehicleEsp_Name = 0;
	int VehicleEsp_Distance = 0;
	int VehicleEsp_3Dbox = 0;
	...
	int Wanted = 0;
	//Aimbot Menu
	float Max_Aimbot_Distance = 100.0f;
	int Aimbot_Toggle = 2;
	int Aimbot_Mode = 0;
	int Aimbot_Type = 0;
	int Aimbot_AutoFire = 0;
	int Aimbot_Silent = 0;
	int Aimbot_StickTo = 0;
	float Aimbot_Fov = 230.0f;
	float Aimbot_Prediction = 0.0200f;
	float Aimbot_Smooth = 10.0f;
	float Wait = 0.0f;
	int Aimbot_Linear = 0;
	int Aimbot_HorizontalOnly = 0;
	int DrawFov = 0;
	int Aim_Until_Dead = 0;
	int Ignore_Vehicle = 0;
	int Aimbot_Bone = 0;
	int InstantKillPed = 0;
	int TargetSelectionAim = 0;

	//Weapon Menu
	int No_Recoil = 0;
	float No_Recoil_Coustum = 0.32f;
	int No_Spread = 0;
	float No_Spread_coustum = 0.5f;
	int Object_Removal_Gun = 0;
	int Fast_Reload = 0;
	int Super_Velocity = 0;
	int Super_Dmg = 0;
	int Super_Impulse = 0;
	int Unlimted_Ammo = 0;
	int Sniper_Range = 0;
	int Rapide_Fire = 0;
	int Bullet_Type = 0;
	int No_Blame_Local = 0;
	int Money_Gun = 0;
	int Fire_Interval = 100;
	int Set_Ammo = 100;

	//Vehicle Menu
	int Veh_God_Mode = 0;
	//float Veh_Impulse = 0.1f;
	float Veh_Impulse = 2.0f;
	int UseLockedCars = 0;
	int DrawLastVeh = 0;
	int DrawPlayerState = 0;
	int RainBowCar = 0;
	int SeatBelt = 0;
	int EngineAlwaysOn = 0;
	int RemoveNearDead = 0;
	int TeleportNearToLoc = 0;
	int SpeedOmeter = 0;
	int FixOnDmg = 0;
	int NoFallOfVeh = 0;
	...
	int Player_ESP = 0;
	//Npc ESP
	float Npc_Max_Distance = 3000.0f;
	int Npc_Name_Esp = 0;
	int Npc_Wanted_Esp = 0;
	int Npc_Box_Esp_Type = 0;
	int Npc_Health_Esp = 0;
	int Npc_Weapon_Esp = 0;
	int Npc_Distance_Esp = 0;
	int Npc_Barrel_Esp = 0;
	int Npc_HeadDot_Esp = 0;
	float Npc_HeadDot_Size = 2;
	float Npc_Line_Size = 1;
	int Npc_Bone_Esp = 0;
	int TargetSelection = 0;

	//2d Radar
	int Toggle_2D_Radar = 0;
	int Center2d = 0;
	int Health2dRadar = 0;
	int BackGround = 0;
	int TargetSelectionRadar = 0;

	float RadarSize = 6.0f;
	float RadarZoom = 6.0f;

	int UseRayCast = 0;

	float Esp_Font_Size = 0.4f;

	float CrossColor[4] = { 1.0f, 0.0f, 0.0f, 1.0f };
	float BgColor[4] = { 0.0f, 0.0f, 0.0f, 0.2f };
	Vector2 RadarLoc = Vector2(100.0f, 100.0f);
};
...
public:
	...
	{
		if (Tools->IsValid(ped_list) && Tools->IsValid(ped_list->peds[index].ped))
			return (ped_list->peds[index].ped);
		else
			return NULL;
	}
	__int32 get_pedHandle(int index)
	{
		if (Tools->IsValid(ped_list) && Tools->IsValid(ped_list->peds[index].ped))
			return (ped_list->peds[index].RetHandle());
		else
			return NULL;
	}
};
...
public:
	...
	{
		if (index <= 0xFF && Tools->IsValid(vehicle_list) && Tools->IsValid(vehicle_list->vehicles[index].vehicle))
...
public:
	...
	{
		if (Tools->IsValid(pCPickupList) && Tools->IsValid(pCPickupList->pickups[index].pCPickup) && pCPickupList->pickups[index].iHandle)
...
public:
	...
	{
		if (Tools->IsValid(pCObjectList) && Tools->IsValid(pCObjectList->ObjectList[index].pCObject) && pCObjectList->ObjectList[index].iHandle)
...
public:
	...
	BYTE GetMaxPassangers() {
		return *reinterpret_cast<BYTE*>((char*)this + 0xC60);
	}

	CObject* GetDriver() {
		return *reinterpret_cast<CObject**>((char*)this + 0xC68);
	}
	...
	void UnLockCar() {
		*reinterpret_cast<BYTE*>((char*)this + 0x1370) = 1; //SET_VEHICLE_DOORS_LOCKED = 1(UnLocked)
		//*reinterpret_cast<BYTE*>((char*)this + 0xA35) = 0; //Is FAHRZEUG Zugang BeiFahrer Locked?
	}
	void LockCar() {
		*reinterpret_cast<BYTE*>((char*)this + 0x1370) = 4; //SET_VEHICLE_DOORS_LOCKED = 1(UnLocked)
		//*reinterpret_cast<BYTE*>((char*)this + 0xA35) = 0; //Is FAHRZEUG Zugang BeiFahrer Locked?
	}

	void SetPosClass(Vector3 Pos) {
		this->fPosition = Pos;
		if (Tools->IsValid(this->_ObjectNavigation))this->_ObjectNavigation->Position = Pos;
	}

	Vector3 V3VehSpeed() {
		//return (*reinterpret_cast<Vector3*>((char*)this + 0xB70));
		return  *reinterpret_cast<Vector3*>((char*)this + OFFvehSpeed);
	}

	//Vector3 V3VehSpeedFiveM() {
	//	return (*reinterpret_cast<Vector3*>((char*)this + 0xB30));
	//}

	void WriteSpeed(Vector3 in) {
		//if(FiveM_Found)
		//	*reinterpret_cast<Vector3*>((char*)this + 0xB30) = in;
		//else
		*reinterpret_cast<Vector3*>((char*)this + 0xB50) = in;
	}
...
public:
	...
	void SetLookRange(float a1) {
		*reinterpret_cast<float*>((char*)this + 0x28C) = a1;
	}

	void SetPenetration(float a1) {
		*reinterpret_cast<float*>((char*)this + 0x108) = a1;
	}

	void SuperRange() {
		*reinterpret_cast<float*>((char*)this + 0x28C) = 999999.0f; //MinRange
	}
	void RapideFire(bool a1) {
		if (a1)
			*reinterpret_cast<float*>((char*)this + 0x134) = 0.001f; //FireRate //0x1DC Animation
		else
			*reinterpret_cast<float*>((char*)this + 0x134) = 1.0f; //FireRate

	}
	void RecoilSet(float val, bool CounterOnly = false) {
		//if(FiveM_Found)
		//	*reinterpret_cast<float*>((char*)this + 0x2E8) = 0.0f;
		//else
		if (CounterOnly) { //Counter recoil
			*reinterpret_cast<float*>((char*)this + OFFrecoil) = -fabsf(*reinterpret_cast<float*>((char*)this + OFFrecoil));
		}
		else {
			*reinterpret_cast<float*>((char*)this + OFFrecoil) = val;
		}
	}

	void SpreadSet(float val) {
		*reinterpret_cast<float*>((char*)this + OFFspread) = val;
		if (OFFspread2 != NULL)
			*reinterpret_cast<float*>((char*)this + OFFspread2) = 0.0f;
	}

	void NoReload() {
		*reinterpret_cast<float*>((char*)this + 0x130) = 5.21f;
		*reinterpret_cast<float*>((char*)this + 0x134) = 5.21f;
	}
	void SuperImpulse() {
		*reinterpret_cast<float*>((char*)this + 0xE0) = 1214.0f; //Vehicles
		*reinterpret_cast<float*>((char*)this + 0xDC) = 1214.0f; //Helis
		*reinterpret_cast<float*>((char*)this + 0xD8) = 1214.0f; //Force Peds
	}
	void SuperDmg() {
		*reinterpret_cast<float*>((char*)this + 0xBC) = 1214.0f;
	}
	void SuperVel() {
		*reinterpret_cast<float*>((char*)this + 0x11C) = 1000000.0f;
	}
	//char* GetSzWeaponName() {
	//	CWeaponName* WeaponName = *reinterpret_cast<CWeaponName**>((char*)this + 0x5F0);
	//	return (Tools->IsValid(WeaponName) ? WeaponName->Name : "Unkown");
	//}

	std::string GetSzWeaponName() {
		CWeaponName* WeaponName = *reinterpret_cast<CWeaponName**>((char*)this + 0x5F0);
		if (Tools->IsValid(WeaponName) && WeaponName->Name && *WeaponName->Name)
...
public:
	...
	void NoPlayerCollision(bool Toggle) {
		DWORD64 addr1 = (DWORD64)_ObjectNavigation;
		if (addr1) {
	...
	bool IsInsideFirstPerson() {
		return *reinterpret_cast<BYTE*>((char*)this + 0x642) ? true : false;
	}
	...
	void Gravity(bool a) {
		if (a) {
			...
			*(DWORD*)((char*)this + 0x600) = 7u;
		}

		//FiveM: 1100 // 1110
		auto navPtr = *(char**)((char*)this + OFFnoGravity);
		if (navPtr) {
			uint16_t flags = *(uint16_t*)((char*)navPtr + 0x1A);

			flags |= 0x10;

			*(uint16_t*)((char*)navPtr + 0x1A) = flags;
		}
	}
	...
	bool IsVisible255() {
		return *reinterpret_cast<BYTE*>((char*)this + 0xAC) ? true : false;
	}

	void ResetHealth() {
		*reinterpret_cast<float*>((char*)this + 0x280) = *reinterpret_cast<float*>((char*)this + 0x2A0);
		//*reinterpret_cast<float*>((char*)this + 0x14B8) = 99.9f;
	}

	void ResetArmour() {
		*reinterpret_cast<float*>((char*)this + OFFarmour) = 99.0f;
	}
	...
	void NoFallOfSeatBelt(bool toggle) {
		if (toggle)
			*(BYTE*)((char*)this + 0x140C) = 0xC9, * (BYTE*)((char*)this + 0x1418) = 0;
		else
			*(BYTE*)((char*)this + 0x140C) = 0xC8, * (BYTE*)((char*)this + 0x1418) = 1;
	}

	void NoRagDoll() {
		*(BYTE*)((char*)this + 0x10B8) = 0;
	}

	unsigned int PedType()
	{
		__int32 Test = *(__int32*)((char*)this + OFFpedType);

		return (Test << 11 >> 25);
	}

	bool IsInVehicle() {
		return (bool)(*reinterpret_cast<__int32*>((char*)this + OFFinVeh) > 0);
		//Is On Foot? = false  --- vehicle = 2
	}

	void LockPlayerInsideVeh() {
	...
	void NoVehDmg() {
		if (Tools->IsValid(gVehicleManager())) {
			//VehicleManager->btGodMode = true;
			gVehicleManager()->fHealth = gVehicleManager()->fHealthMax;
			*reinterpret_cast<BYTE*>((char*)gVehicleManager() + 0x9D4) = 39; //Bullet proof tires.
			*reinterpret_cast<float*>((char*)gVehicleManager() + 0x9D8) = 0.f; //Dirt
		}
	}
...
public:
	...
	{
		return Tools->IsValid(pLocalPlayer) ? pLocalPlayer : nullptr;
	}
...
class Addresses {
public:
	CWorld* LocalPlayerWrapper = NULL; //48 8B 05 ? ? ? ? 45 0F C6 C9
	CBlipList* WayPointBlipList = NULL; //48 8B 05 ? ? ? ? 45 0F C6 C9
	CPlayers* playerList = NULL; //48 8B 0D ? ? ? ? E8 ? ? ? ? 48 8B C8 E8 ? ? ? ? 48 8B CF
	CObject* localPlayer = NULL; //Just Define Local Player as CObject no Sig needded
	CGameCameraAngles* pGameCamManager = NULL; //? ? ? ? ? ? ? 4A 8B 1C F0 48 85
	CReplayInterface* ReplayInterface = NULL;
	//RemotePlayer* RemotePlayerRageMP = NULL;
	DWORD64 BASE = 0x0;
	DWORD64 WayPointRead = 0x0;
};
static Addresses addy;
...
static bool VK_LBUTTON_PRESS = false;
